...
def book_slot(event_id: int, payload: BookingIn, user=Depends(get_current_user)):
    conn = db.get_connection()
    cur = conn.cursor()
    slot = cur.execute(
        "SELECT * FROM slots WHERE id = ? AND eventId = ?", (payload.slotId, event_id)
    ).fetchone()
    if not slot:
        conn.close()
        raise HTTPException(404, "Slot not found")
    count = cur.execute(
        "SELECT COUNT(*) AS cnt FROM bookings WHERE slotId = ?", (payload.slotId,)
    ).fetchone()["cnt"]
    ...
    if cur.execute(
        ...
        (payload.slotId, payload.email),
    ).fetchone():
        conn.close()
        raise HTTPException(
            status_code=400,
            detail=f"You cannot book again: this email ({payload.email}) has already been used for this event."
        )
    cur.execute(
        "INSERT INTO bookings (slotId, name, email) VALUES (?, ?, ?)",
        (payload.slotId, payload.name, payload.email),
    )
